...
def create_job_View(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            form.save_m2m()
            messages.success(
                    request, 'Sua vaga foi anunciada com sucesso! Aguarde a análise.')
...
def apply_job_view(request, id):
    ...
    if not applicant:
        ...
        messages.error(request, 'Você já se inscreveu para o emprego!')
...
def job_edit_view(request, id=id):
    ...
    if form.is_valid():
        ...
        # for save tags
        # form.save_m2m()
        messages.success(request, 'Seu anúncio de emprego foi atualizado com sucesso!')
        return redirect(reverse("jobapp:single-job", kwargs={
            'id': instance.id
        }))
    context = {
